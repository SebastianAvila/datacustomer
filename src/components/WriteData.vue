    ...
    <v-row class="text-center">
      <v-col cols="12">
        <v-card>
          ...
          <v-skeleton-loader v-if="loading" type="table"></v-skeleton-loader> <!-- Skeleton Loader para la tabla -->
          <!-- Mostrar la tabla cuando loading sea false -->
          <v-data-table v-if="!loading" :headers="headers" :items="Records" :items-per-page="5" :footer-props="{
      ...
      <v-dialog v-model="dialog" width="800">
        <v-skeleton-loader v-if="loading" type="text"></v-skeleton-loader> <!-- Skeleton Loader para el v-dialog -->

        <v-card v-if="!loading"> <!-- Mostrar el v-dialog cuando loading sea false -->
...
export default {
  ...
  methods: {
    ...
    // Obtiene la data al hacer clic consumiendo la otra API
    async getSecondData(item) {
      ...
      try {
        ...
        if (response.ok) {
          ...
          // Accede a IdSatCode dentro de Records y lo asigna
          if (responseData.Records && responseData.Records.length > 0) {
            ...
            // Si no hay datos en Records, muestra un mensaje de error
